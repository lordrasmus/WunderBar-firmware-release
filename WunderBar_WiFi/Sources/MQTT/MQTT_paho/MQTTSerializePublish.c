...
/**
  ...
  */
int MQTTSerialize_ack(char* buf, int buflen, int type, int packetid, int dup)
{
	MQTTHeader header;
	int rc = 0;
	char *ptr = buf;
...
/**
  ...
  * @return serialized length, or error if 0
  */
int MQTTSerialize_pubrec(char* buf, int buflen, int packetid)
{
	return MQTTSerialize_ack(buf, buflen, PUBREC, packetid, 0);
}
