...
#define IS_FLAG_LONG(a)     (a & FLAGS_LONG)
...
printk_mknumstr (char *numstr, void *nump, int neg, int radix)
{
    ...
    *nstrp++ = '\0';
    ...
    if (neg)
    {
        ...
        while (a != 0)
        {
            b = (long long int)a / (int)radix;
            c = (long long int)a - ((long long int)b * (int)radix);
            if (c < 0)
            {
                c = ~c + 1 + '0';
            }
    ...
    else
    {
        ...
        while (ua != 0)
        {
            ub = (unsigned long long int)ua / (unsigned int)radix;
            uc = (unsigned long long int)ua - ((unsigned long long int)ub * (unsigned int)radix);
            if (uc < 10)
            {
                uc = uc + '0';
            }
        ...
        switch (c = *++p)
        {
            ...
            case 'u':
            	if (IS_FLAG_LONG(flags_used))
            		...
            		ival = (unsigned int)va_arg(ap, unsigned int);
