 ...
 */
bool GS_API_StartProvisioning(char* provSsid, char* provChannel, char* ip, char* subnetMask, char* hostName);
...
/**
   @brief Set up the default saved network

   @return true if network connection was set
 */
bool GS_API_SetupWifiNetwork(HOST_APP_NETWORK_CONFIG_T* apiNetCfg);

/**
   @brief Joins already configured wifi network

   @return true if network connection joined
 */
...
/**
   @brief Checks for network association and confirms wifi ssid
...
/**
   @brief performs dns look up for desired url address

   Returns IP address for requested url

   @param Pointer to desired url address
   @param ipAddr Pointer to store IP address as a string. Needs to be at least GS_API_IP_STR_LENGTH long.
 */
