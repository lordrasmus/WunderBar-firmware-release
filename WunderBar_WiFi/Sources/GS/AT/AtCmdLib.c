 ...
 * Constants:
 *-------------------------------------------------------------------------*/
volatile unsigned int AtLib_Response_Handle_Timeout = 30000;	/* ms */
...
/*---------------------------------------------------------------------------*
 * Routine:  AtLibGs_WlanConnStatShort
 *---------------------------------------------------------------------------*
 * Description:
 *      Get the network MAC address for this module.
 *      Sends the command:
 ...
 *      and waits for a response.
 *      Use the routine AtLib_ParseNodeIpAddress() or
 *      AtLib_ParseSSIDResponse() to parse the returned data for SSID
 * Inputs:
 *      void
 * Outputs:
 *      HOST_APP_MSG_ID_E -- response type
 *---------------------------------------------------------------------------*/
HOST_APP_MSG_ID_E
AtLibGs_WlanConnStatShort (void)
{
  HOST_APP_MSG_ID_E rxMsgId;

  /* Construct the AT command */
  sprintf (G_ATCmdBuf, "AT+WSTATUS\n");
		    ...
		    uint32_t alarm1_Pol, uint32_t alarm2_Pol)
{
  ...
  sprintf (G_ATCmdBuf,
	   ...
	   (int)dealy, (int)alarm1_Pol, (int)alarm2_Pol);
		   ...
		   uint32_t srcPort, int8_t * pSrcIP)
{
  ...
  sprintf (G_ATCmdBuf, "AT+FWUP=%s," _F32_ "," _F32_ ",%s\r\n", (char*)pSrvip,
		  (int)srvport, (int)srcPort, (char*)pSrcIP);
 ...
 *---------------------------------------------------------------------------*/
HOST_APP_MSG_ID_E
AtLibGs_SetTime (int8_t * pTime)
{
  HOST_APP_MSG_ID_E rxMsgId;

  /* Construct the AT command */
  sprintf (G_ATCmdBuf, "AT+SETTIME=%s\n", pTime);
 ...
 *---------------------------------------------------------------------------*/
bool
AtLib_ParseWlanConnIp (uint8_t* response)
{
 uint8_t cnt;
 uint8_t txt[17], *ptr1, *ptr2;

 txt[16] = 0;
 ptr2 = &txt[15];
 cnt = 0;

 if (strstr((const char *) MRBuffer, "    IP    "))
 {
  if ((ptr1 = (uint8_t *) strstr ((const char *) MRBuffer, ": ")))
    {
	  ptr1 --;
	  while (*ptr1 != ' ' || cnt > 16)
	   {
	  ...
	  strcpy((char *) response, (const char *) ++ ptr2);
	  return true;
    }
 }
...
/*---------------------------------------------------------------------------*
 * Routine:  AtLib_ParseSSIDResponse
 *---------------------------------------------------------------------------*
 * Description:
 *      Parses the last line returned after calling AT+WSTATUS.
 *      If a connection is successful return ssid.  Otherwise, responds
 *      with false.
 * Inputs:
 *      return value of ssid
 * Outputs:
 *      uint8_t -- true if successful.
 *---------------------------------------------------------------------------*/
bool
AtLib_ParseSSIDResponse(uint8_t* ssid)
{
	uint8_t *ptr;

	if ((ptr = (uint8_t *) strstr((const char *) MRBuffer, (const char *) "SSID:\"")) > 0)
	{
		ptr = (uint8_t *) strchr((const char *) ptr, (int) '\"');
		ptr ++;
		 while ( *ptr != '\"' )
		 {
			 *ssid ++ = *ptr ++;
 ...
 *---------------------------------------------------------------------------*/
uint8_t
AtLib_ParseDNSLookupResponse (uint8_t *ipAddr)
{
  char *pSubStr = NULL, *ptr = NULL;

  pSubStr = strstr ((const char *) MRBuffer, "IP:");
  if (pSubStr)
    {
      ptr = strchr ((const char *) pSubStr, '\r');
      *ptr = '\0';
	  strcpy ((char *) ipAddr, (pSubStr + 3));
      return 1;			/* Success */
    }
  else
    {
...
{
  GS_HAL_send ((uint8_t *) pTxData, dataLen);
}
...
AtLib_BulkDataTransfer (uint8_t cid, const uint8_t * pData, uint32_t dataLen)
{
  ...
  /* Now send the actual data */
  GS_HAL_send ((uint8_t *) pData, dataLen);
...
AtLib_getCidFromAscii (uint8_t cidAscii)
{
  ...
  sscanf ((const char *) cidStr, "%x", (unsigned int *) &temp);
...
AtLib_ReceiveDataProcess (uint8_t rxData)
{
  ...
  switch (receive_state)
      ...
      while (specialDataLen)
	{
      ...
      AtLib_ProcessCompletedHttpBulkTransferEvent(rxCurrentCid);
      ...
      while (specialDataLen)
	{
      ...
      AtLib_ProcessCompletedBulkTransferEvent(rxCurrentCid);
 ...
 * Outputs:
 *      HOST_APP_MSG_ID_E -- error code
 *---------------------------------------------------------------------------*/
HOST_APP_MSG_ID_E AtLib_Ping(
        uint8_t ip[],
        uint16_t trails,
        uint16_t timeout,
        uint16_t len,
        ...
        char* cert)
{
    ...
    sprintf(G_ATCmdBuf, "AT+TCERTADD=%s," _F8_ "," _F16_ "," _F8_ "\n", name, hex,
            size, ram);
    ...
    sprintf(G_ATCmdBuf, "%c%c", HOST_APP_ESC_CHAR, 'W');

    GS_HAL_send((uint8_t *) & G_ATCmdBuf[0], 1);
    GS_HAL_send((uint8_t *) & G_ATCmdBuf[1], 1);

    while (size --)
    	GS_HAL_send((uint8_t *) cert ++, 1);
...
/*---------------------------------------------------------------------------*
 * Routine:  AtLib_DeleteSSLCertificate
 *---------------------------------------------------------------------------*
 * Description:
 ...
 *---------------------------------------------------------------------------*/
HOST_APP_MSG_ID_E
AtLib_DeleteSSLCertificate(char name[])
{
    HOST_APP_MSG_ID_E rxMsgId;
 ...
 *---------------------------------------------------------------------------*/
HOST_APP_MSG_ID_E
AtLib_SSLOpen(uint8_t cid, char caName[])
{
    HOST_APP_MSG_ID_E rxMsgId;

    sprintf(G_ATCmdBuf, "AT+SSLOPEN=" _F8_ ",%s\n", (int)cid, caName);
 ...
 *---------------------------------------------------------------------------*/
HOST_APP_MSG_ID_E
AtLib_SSLClose(uint8_t cid)
{
    HOST_APP_MSG_ID_E rxMsgId;

    sprintf(G_ATCmdBuf, "AT+SSLCLOSE=" _F16_ "\r\n", cid);
 ...
 *---------------------------------------------------------------------------*/
HOST_APP_MSG_ID_E
AtLib_HTTPConf(ATLIB_HTTPCLIENT_E param, char value[])
{
	HOST_APP_MSG_ID_E rxMsgId;

    sprintf(G_ATCmdBuf, "AT+HTTPCONF=" _F16_ ",", param);
	
    GS_HAL_send((uint8_t *) G_ATCmdBuf, strlen(G_ATCmdBuf));
    GS_HAL_send((uint8_t *) value, strlen(value));
    GS_HAL_send((uint8_t *) "\r\n", 2);
 ...
 *---------------------------------------------------------------------------*/
HOST_APP_MSG_ID_E
AtLib_HTTPConfDel(ATLIB_HTTPCLIENT_E param)
{
	HOST_APP_MSG_ID_E rxMsgId;

    sprintf(G_ATCmdBuf, "AT+HTTPCONFDEL=" _F16_ "\r\n", param);
 ...
 *---------------------------------------------------------------------------*/
HOST_APP_MSG_ID_E
AtLib_HTTPClose(uint8_t cid)
{
	HOST_APP_MSG_ID_E rxMsgId;

    /* TODO: Is CID a character or a number? */
    sprintf(G_ATCmdBuf, "AT+HTTPCLOSE=" _F8_ "\r\n", cid);
        ...
        uint8_t* cid)
{
    ...
    sprintf(G_ATCmdBuf, "AT+HTTPOPEN=%s," _F16_ "\n", host, port);
...
AtLibGs_HTTPSend(
        uint8_t cid,
        ATLIB_HTTPSEND_E type,
        uint16_t timeout,
        char page[],
        uint16_t size,
        const void *txBuf)
{
	HOST_APP_MSG_ID_E rxMsgId = HOST_APP_MSG_ID_INVALID_INPUT;
    if (HOST_APP_INVALID_CID != cid) {

    	if (type == ATLIBGS_HTTPSEND_GET)
    	{
    		sprintf(G_ATCmdBuf, "AT+HTTPSEND=" _F8_ "," _F8_ "," _F16_ ",%s\n", cid, type,
    				timeout, page);
    	}
    	else
    	{
            sprintf(G_ATCmdBuf, "AT+HTTPSEND=" _F8_ "," _F8_ "," _F16_ ",%s" "," _F16_ "\n", cid, type,
                    timeout, page, size);
    	}
        ...
        rxMsgId = AtLib_ResponseHandle();
        if (rxMsgId == HOST_APP_MSG_ID_OK) {
        	if ((type == ATLIBGS_HTTPSEND_POST) && (size))
        	{
				/* Construct the data start indication message */
				sprintf(G_ATCmdBuf, "%c%c" _F8_, HOST_APP_ESC_CHAR, 'H', cid);
				/* Now send the data START indication message  to S2w node */
				GS_HAL_send((uint8_t *) G_ATCmdBuf, 3);
				/* Now send the actual data */
				GS_HAL_send((uint8_t *) txBuf, size);
        	}
        }
    }
    return rxMsgId;
...
/*---------------------------------------------------------------------------*
 * Routine:  AtLib_ParseSystemTime
 *---------------------------------------------------------------------------*
 * Description:
 *      Parses the last line returned after doing a AtLib_GetTime()
 *      command.  Return unix time in string from GS module
 * Inputs:
 *      pointer to string where time will be stored
 * Outputs:
 *      0   - error
 *      1   - success
 *---------------------------------------------------------------------------*/
uint8_t
AtLib_ParseSystemTime(char* time)
{
	int8_t count;
	char *pSubStr = NULL;
	// find second comma in text
	if ((pSubStr = strchr ((const char *) MRBuffer, ',')) == NULL)
		return 0;

	if ((pSubStr = strchr ((const char *) ++ pSubStr, ',')) == NULL)
		return 0;
 ...
 *---------------------------------------------------------------------------*/
HOST_APP_MSG_ID_E
AtLib_SetGPIO( ATLIB_GPIO_PIN_E gpio,
				ATLIB_GPIO_STATE_E state)
{
    HOST_APP_MSG_ID_E rxMsgId;

    sprintf(G_ATCmdBuf, "AT+DGPIO=" _F16_ "," _F8_ "\r\n", gpio, state);
 ...
 *---------------------------------------------------------------------------*/
HOST_APP_MSG_ID_E
AtLib_GetCIDInfo(void)
{
    HOST_APP_MSG_ID_E rxMsgId;
    /* TODO: Parse CID response! */
    sprintf(G_ATCmdBuf, "AT+CID=?\r\n");
 ...
 *     AT+CID=?
 * Inputs:
 *    uint32_t mode -- n seconds for keep alive
 * Outputs:
 *      ATLIBGS_MSG_ID_E -- error code
 *---------------------------------------------------------------------------*/
HOST_APP_MSG_ID_E
AtLib_WLanKeepalive(uint32_t seconds)
{
    HOST_APP_MSG_ID_E rxMsgId;

    sprintf(G_ATCmdBuf, "AT+WKEEPALIVE=" _F32_ "\r\n", (int) seconds);
 ...
 *---------------------------------------------------------------------------*/
HOST_APP_MSG_ID_E
AtLib_GetMemoryInfo(void)
{
    HOST_APP_MSG_ID_E rxMsgId;
    /* TODO: Parse CID response! */
    sprintf(G_ATCmdBuf, "AT+MEMTRACE/r/n");
 ...
 *---------------------------------------------------------------------------*/
HOST_APP_MSG_ID_E
AtLib_SetPhyMode (uint32_t mode)
{
  HOST_APP_MSG_ID_E rxMsgId;

  /* Construct the AT command */
  sprintf (G_ATCmdBuf, "AT+WPHYMODE=" _F32_ "\r\n", (int) mode);
 ...
 *      and waits for a response.
 * Inputs:
 *      uint32_t mode -- 1=PCB antenna, 2=uFL
 * Outputs:
 *      HOST_APP_MSG_ID_E -- response type
 *---------------------------------------------------------------------------*/
HOST_APP_MSG_ID_E
AtLib_SetAntennaConf (uint32_t mode)
{
  HOST_APP_MSG_ID_E rxMsgId;

  /* Construct the AT command */
  sprintf (G_ATCmdBuf, "AT+ANTENNA=" _F32_ "\r\n", (int) mode);
 ...
 *---------------------------------------------------------------------------*/
HOST_APP_MSG_ID_E
AtLib_GetPhyMode(void)
{
    HOST_APP_MSG_ID_E rxMsgId;

    sprintf(G_ATCmdBuf, "AT+WPHYMODE=?\r\n");
...
/*---------------------------------------------------------------------------*
 * Routine:  AtLib_EnableSoftFlowControl
 *---------------------------------------------------------------------------*
 * Description:
 *      Set mode to Infrastructure or Ad-Hoc
 *      Sends the command:
 ...
 *---------------------------------------------------------------------------*/
HOST_APP_MSG_ID_E
AtLib_EnableSoftFlowControl (uint32_t mode)
{
  HOST_APP_MSG_ID_E rxMsgId;

  /* Construct the AT command */
  sprintf (G_ATCmdBuf, "AT&K" _F32_ "\r\n", (int) mode);
 ...
 *---------------------------------------------------------------------------*/
HOST_APP_MSG_ID_E
AtLib_PsPoll (uint32_t mode, uint32_t interval)
{
  HOST_APP_MSG_ID_E rxMsgId;

  /* Construct the AT command */
  sprintf (G_ATCmdBuf, "AT+WIEEEPSPOLL=" _F32_ "," _F32_ "\r\n", (int) mode, (int) interval);
...
/*---------------------------------------------------------------------------*
 * Routine:  AtLib_SetSocketOptions
 *---------------------------------------------------------------------------*
 * Description:
 ...
 * Inputs:
 *      ATLIB_SOCKET_OPTION_TYPE_E type -- Type of socket option
 *          (socket, IP, TCP)
 *      ATLIB_SOCKET_OPTION_PARAM_E param -- Parameter type being set
 *          (TCP MAXRT, TCP_KA, SO_KA, TCO_KA_CNT)
 *      uint32_t value -- Value to be set in seconds
 * Outputs:
 *      ATLIBGS_MSG_ID_E -- error code
 *---------------------------------------------------------------------------*/
HOST_APP_MSG_ID_E AtLib_SetSocketOptions(
        uint8_t cid,
        ATLIB_SOCKET_OPTION_TYPE_E type,
        ATLIB_SOCKET_OPTION_PARAM_E param,
        uint32_t value)
{
    ...
    sprintf(
            ...
            cid, type, param, (int) value, length);
...
/*---------------------------------------------------------------------------*
 * Routine:  AtLib_ProcessCompletedBulkTransferEvent
 *---------------------------------------------------------------------------*
 * Description:
 *      Generates an event when bulk data is received.
 * Inputs:
 *      cid
 * Outputs:
 *      void
 *---------------------------------------------------------------------------*/
void
AtLib_ProcessCompletedBulkTransferEvent (uint8_t cid)
{
	App_ProcessCompletedBulkTransferEvent(cid);
}

/*---------------------------------------------------------------------------*
 * Routine:  AtLib_ProcessCompletedHttpBulkTransferEvent
 *---------------------------------------------------------------------------*
 * Description:
 *      Generates an event when bulk data is received.
 * Inputs:
 ...
 *---------------------------------------------------------------------------*/
void
AtLib_ProcessCompletedHttpBulkTransferEvent (uint8_t cid)
{
	App_ProcessCompletedHttpBulkTransferEvent(cid);
}
