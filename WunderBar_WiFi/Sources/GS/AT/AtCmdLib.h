...
extern volatile unsigned int AtLib_Response_Handle_Timeout;
...
typedef enum {
    ATLIB_SOCKET_OPTION_TYPE_SOCK = 65535,
    ATLIB_SOCKET_OPTION_TYPE_IP = 0,
    ATLIB_SOCKET_OPTION_TYPE_TCP = 6,
    ATLIB_SOCKET_OPTION_TYPE_UNKNOWN = -1
} ATLIB_SOCKET_OPTION_TYPE_E;

typedef enum {
    ATLIB_SOCKET_OPTION_PARAM_TCP_MAXRT = 10,
    ATLIB_SOCKET_OPTION_PARAM_TCP_KEEPALIVE = 4001,
    ATLIB_SOCKET_OPTION_PARAM_SO_KEEPALIVE = 0x8,
    ATLIB_SOCKET_OPTION_PARAM_TCP_KEEPALIVE_COUNT = 4005,
    ATLIB_SOCKET_OPTION_PARAM_UNKNOWN = -1
} ATLIB_SOCKET_OPTION_PARAM_E;
...
HOST_APP_MSG_ID_E AtLibGs_WlanConnStatShort (void);
...
HOST_APP_MSG_ID_E AtLibGs_SetTime(int8_t *pTime);
HOST_APP_MSG_ID_E AtLib_GetTime(void);
...
HOST_APP_MSG_ID_E AtLibGs_HTTPOpen(char host[], uint16_t port, uint8_t* cid);
...
HOST_APP_MSG_ID_E AtLib_HTTPClose(uint8_t cid);

HOST_APP_MSG_ID_E AtLib_HTTPConf(ATLIB_HTTPCLIENT_E param, char value[]);
HOST_APP_MSG_ID_E
AtLibGs_HTTPSend(
        uint8_t cid,
        ...
        uint16_t size,
        const void *txBuf);
HOST_APP_MSG_ID_E AtLib_SetGPIO(ATLIB_GPIO_PIN_E gpio, ATLIB_GPIO_STATE_E state);
HOST_APP_MSG_ID_E AtLib_EnableSoftFlowControl (uint32_t mode);
HOST_APP_MSG_ID_E AtLib_WLanKeepalive(uint32_t seconds);
HOST_APP_MSG_ID_E AtLib_PsPoll (uint32_t mode, uint32_t interval);

bool AtLib_ParseWlanConnIp (uint8_t* response);
bool AtLib_ParseSSIDResponse(uint8_t* ssid);
...
uint8_t AtLib_ParseDNSLookupResponse(uint8_t *ipAddr);
...
uint8_t AtLib_ParseSystemTime(char* time);
HOST_APP_MSG_ID_E AtLibGs_Ping(
        char ip[],
        uint16_t trails,
        uint16_t timeout,
        uint16_t len,
...
HOST_APP_MSG_ID_E AtLib_SetAntennaConf (uint32_t mode);
HOST_APP_MSG_ID_E AtLib_SetSocketOptions(uint8_t cid, ATLIB_SOCKET_OPTION_TYPE_E type, ATLIB_SOCKET_OPTION_PARAM_E param, uint32_t value);

HOST_APP_MSG_ID_E AtLib_Ping(uint8_t ip[], uint16_t trails, uint16_t timeout, uint16_t len, uint8_t tos, uint8_t ttl, uint8_t payload[]);
...
HOST_APP_MSG_ID_E AtLib_SSLClose(uint8_t cid);
HOST_APP_MSG_ID_E AtLib_SSLOpen(uint8_t cid, char caName[]);
HOST_APP_MSG_ID_E AtLib_DeleteSSLCertificate(char name[]);
HOST_APP_MSG_ID_E AtLib_AddSSLCertificate(char name[], bool hex, uint16_t size, bool ram, char* cert);
...
HOST_APP_MSG_ID_E AtLib_GetMemoryInfo(void);
HOST_APP_MSG_ID_E AtLib_GetPhyMode(void);
HOST_APP_MSG_ID_E AtLib_SetPhyMode (uint32_t mode);
HOST_APP_MSG_ID_E AtLib_GetCIDInfo(void);
...
void AtLib_ProcessCompletedHttpBulkTransferEvent (uint8_t cid);
void AtLib_ProcessCompletedBulkTransferEvent(uint8_t cid);
// User supplied routines
extern void App_ProcessIncomingData(uint8_t cid, uint8_t rxData);
extern void App_ProcessCompletedBulkTransferEvent(uint8_t cid);
extern void App_ProcessCompletedHttpBulkTransferEvent(uint8_t cid);
