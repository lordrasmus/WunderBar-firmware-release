...
#define PSTORAGE_DRIVER_NUM_OF_BLOCKS     8           /**< Number of blocks requested by the module. */
    ...
    case PSTORAGE_UPDATE_OP_CODE:
        if (result == NRF_SUCCESS) 
        {
            ...
            if(
                ...
                (pstorage_driver_get_run_status() == false) &&
								// Check if sensor are in some or storing state of onboarding mode.
                ((onboard_get_state() == ONBOARD_STATE_STORE_MODE) || (onboard_get_state() == ONBOARD_STATE_WAIT_TO_STORE_MODE))
              )
            {
