...
/** @file   main.c
 *  @brief  This file contains the source code for firmware of IR TRANSMITTER sensor of Wunderbar Board.
 *
...
const uint16_t characteristic_sensorData_uuid             = CHARACTERISTIC_SENSOR_DATA_W_UUID;               /**< 16-bit UUID value for data down char. */
...
ble_characteristic_info_t characteristic_sensorData_info;
...
#define IR_OUT_PIN 25                  /**< Number of pin which is connected to IR diode. */   
#define IR_RGB_ADDR 0x00               /**< RGB Address. */

sensor_ir_t sensor_ir;
...
const uint8_t                DEFAULT_DEVICE_NAME[BLE_DEVNAME_MAX_LEN+1] = DEVICE_NAME_IR;
const sensorID_t             DEFAULT_SENSOR_ID = {0x66, 0x66, 0x66, 0x66, 0x66, 0x66, 0x66, 0x66,
                                                  0x66, 0x66, 0x66, 0x66, 0x66, 0x66, 0x66, 0x66};
const beaconFrequency_t      DEFAULT_SENSOR_BEACON_FREQUENCY            = ADV_INTERVAL_MS;
const bool                   DEFAULT_SENSOR_LED_STATE                   = false;
...
/** @brief  This macro sends logical high level. */
#define HI()                        \
            do                      \
            {                       \
                ir_pulse(22);       \
...
/** @brief  This macro sends logical low level. */
#define LO()                       \
            do                     \
            {                      \
                ir_pulse(22);      \
...
{
    server_def.passkey = sensor_ir.passkey;
    sensor_ir.led_state = DEFAULT_SENSOR_LED_STATE;
...

/**@brief   This function sends one IR command.
 *
...
void ir_send_cmd(uint8_t command)
{
    sensor_ir.data = command;
    ir_pulse(346);
		nrf_delay_us(4500);
    ir_send_byte(IR_RGB_ADDR);
...

/**@brief   This function sends one byte through IR interface.
 *
...

/**@brief   This function generate "count" number of pulses, with corresponding duration.
 *
 * @param   count Number of pulses to be generated.
 *
 * @return  Void.
 */
...
void ir_pulse(int count) 
{
    ...
    for (i=0; i<count;i++) 
    {
        gpio_write(IR_OUT_PIN, true);
				nrf_delay_us(16);
        gpio_write(IR_OUT_PIN, false);
        nrf_delay_us(7);
    }
}
...
{ 
    ...
    if(!init_global((uint8_t*)&sensor_ir.sensorID, (uint8_t*)&DEFAULT_SENSOR_ID, sizeof(sensor_ir.sensorID))) 
    {
        return false;
    }
    
    // Read SensorBeaconFrequency from p_storage.
    if(!init_global((uint8_t*)&sensor_ir.beaconFrequency, (uint8_t*)&DEFAULT_SENSOR_BEACON_FREQUENCY, sizeof(sensor_ir.beaconFrequency))) 
    {
        return false;
    }
    
    // Read Passkey from p_storage.
    if(!init_global((uint8_t*)sensor_ir.passkey, (uint8_t*)DEFAULT_SENSOR_PASSKEY, sizeof(sensor_ir.passkey))) 
    {
        return false;
    }
    
    // Read mitm_req_flag from p_storage.
    if(!init_global((uint8_t*)&sensor_ir.mitm_req_flag, (uint8_t*)&DEFAULT_MITM_REQ_FLAG, sizeof(sensor_ir.mitm_req_flag))) 
    {
        return false;
    }
...
{  
    ...
    {
      ble_start_advertising(sensor_ir.beaconFrequency);
...
{ 
    ...
    ble_dispatch_write_characteristic(evt_write->handle, evt_write->offset, evt_write->len, evt_write->data, &characteristic_sensorData_info);  
...
{ 
    ...
    if ((char_info == &characteristic_sensorID_info) && (offset == 0) && (len == sizeof(sensor_ir.sensorID))) 
    {
			  // Copy new data to local buffer, and store it to persistent memory.
        memcpy((uint8_t*)&sensor_ir.sensorID, data, len); 
        pstorage_driver_request_store((uint8_t*)(&sensor_ir.sensorID));
    }
    
		// Write to beacon frequency characteristic value?
    if ((char_info == &characteristic_sensorBeaconFrequency_info) && (offset == 0) && (len == sizeof(sensor_ir.beaconFrequency))) 
    {
        ...
        {
            sensor_ir.beaconFrequency = tmp_beaconFrequency;
            pstorage_driver_request_store((uint8_t*)(&sensor_ir.beaconFrequency));
        }
    }
    
		// Write to led state characteristic value?
    if ((char_info == &characteristic_sensorLedState_info) && (offset == 0) && (len == sizeof(sensor_ir.led_state))) 
    {
			  // Copy new data to local buffer, and update state of LED diode.
        memcpy((uint8_t*)&sensor_ir.led_state, data, len);  
			  led_control_update_char(sensor_ir.led_state, LED_TIMEOUT_CHAR_MS);
    }
    
		// Write to data down characteristic value?
    if ((char_info == &characteristic_sensorData_info) && (offset == 0) && (len == 1)) 
    {
			  // Send received data through IR.
        ir_send_cmd(*data);
    }
    ...
    {
			  // Copy new data to local buffer, and store it to persistent memory.
        memcpy((uint8_t*)&sensor_ir.passkey, data, len);  
        pstorage_driver_request_store((uint8_t*)(&sensor_ir.passkey));
    ...
    if ((char_info == &characteristic_sensorMitmReqFlag_info) && (offset == 0) && (len == sizeof(sensor_ir.mitm_req_flag)))
    {
			  // Copy new data to local buffer, and store it to persistent memory.
        sensor_ir.mitm_req_flag = (data[0] == 1);
        pstorage_driver_request_store((uint8_t*)(&sensor_ir.mitm_req_flag));
    }
}
...
{ 
    ...
    gpio_write(IR_OUT_PIN, false);                                                                 // Turn off IR pin.
    gpio_set_pin_digital_output(IR_OUT_PIN, PIN_DRIVE_S0S1);                                       // Set IR pin to be output.
  
	  // Initialize some global variables.  
    init_globals();
  
	  // Initialize BLE server modules.
    if (!ble_init_server(&server_def, pstorage_driver_init, &sensor_ir.mitm_req_flag)) 
    { 
        blink(101); 
    }
    ...
    {     
        ...
        uint16_t read_enc_flag  = (sensor_ir.mitm_req_flag ? BLE_CHARACTERISTIC_READ_ENC_REQUIRE  : BLE_CHARACTERISTIC_READ_ENC_REQUIRE_NO_MITM);
        uint16_t write_enc_flag = (sensor_ir.mitm_req_flag ? BLE_CHARACTERISTIC_WRITE_ENC_REQUIRE : BLE_CHARACTERISTIC_WRITE_ENC_REQUIRE_NO_MITM);
        
        short_service_uuid = sensor_ir.mitm_req_flag ? short_service_relayr_uuid : short_service_relayr_open_comm_uuid; 
        ...
        if (!ble_add_characteristic(&service_info,
                                ...
                                (const uint8_t*)sensor_ir.sensorID,
                                sizeof(sensorID_t),
        ...
        if (!ble_add_characteristic(&service_info,
                                    ...
                                    (const uint8_t*)&sensor_ir.beaconFrequency,
                                    sizeof(sensor_ir.beaconFrequency),
        ...
        if (!ble_add_characteristic(&service_info,
                                    ...
                                    (const uint8_t*)&sensor_ir.led_state,
                                    sizeof(sensor_ir.led_state),
        ...
        if (!ble_add_characteristic(&service_info,
                                    characteristic_sensorData_uuid,
                                    BLE_CHARACTERISTIC_CAN_WRITE | read_enc_flag,
                                    (const uint8_t*)"SensorData", 
                                    (const uint8_t*)&sensor_ir.data,
                                    sizeof(sensor_ir.data),
                                    &characteristic_sensorData_info)) 
        {
            blink(104);
        }
    }
    ...
    else
    {
        ...
        if (!ble_add_characteristic(&service_info,
                                    ...
                                    (const uint8_t*)sensor_ir.passkey,
                                    6,
        ...
        if (!ble_add_characteristic(&service_info,
                                    ...
                                    (const uint8_t*)&sensor_ir.mitm_req_flag,
    ...
    if (!ble_init_advertising()) 
    {
		...
		// Start advertising.
    if (!ble_start_advertising(sensor_ir.beaconFrequency)) 
    {
        blink(106);
    }
